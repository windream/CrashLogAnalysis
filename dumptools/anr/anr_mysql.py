...
nativeSharedDirty,otherSharedDirty,logName,dn)

def anr_mysql_insert(src_fn,version,dn):
	try:
		conn = MySQLdb.connect(host = "localhost",user = "root",passwd = "123",db = "DumpLog",charset = "utf8")
		cursor = conn.cursor()
		sql = "insert into Anr(me,buildno,channel,appflags,debug,imei,board,bootloader,brand,cpu_abi,cpu_abi2,device,display,fingerprint,hardware,host,id,manufacturer,\
		model,product,radio,tags,type,user,codename,incremental,myrelease,sdk,dalvikPss,nativePss,otherPss,dalvikPrivateDirty,\
		nativePrivateDirty,otherPrivateDirty,dalvikSharedDirty,nativeSharedDirty,otherSharedDirty,logName,logLocation) \
